...
def generate_launch_description():
    ...
    launch_args = [
        DeclareLaunchArgument(
            "urdf_file", default_value=os.path.join(desc_dir, "urdf", "go2.urdf.xacro")
        ),
    ...
    spawn_go2_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([desc_dir, "launch", "gazebo", "spawn_go2.launch.py"])
        ),
        launch_arguments={"x": x, "y": y, "z": z, "yaw": yaw, "robot_up": robot_up, "robot_base_link": LaunchConfiguration("robot_base_link")}.items(),
    )
